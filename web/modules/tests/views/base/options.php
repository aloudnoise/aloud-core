...
<div class="action-content">
    ...
    $f = \app\widgets\EForm\EForm::begin([
        "htmlOptions"=>[
            "action"=>app\helpers\OrganizationUrl::to(array_merge(["/tests/base/options"], \Yii::$app->request->get())),
            "method"=>"post",
            "id"=>"testOptionsForm"
        ],
    ]);
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <?php if (!empty($criteria_templates)) { ?>
                            <?php foreach ($criteria_templates as $template) { ?>
                                <a target="modal" class="dropdown-item" href="<?=\app\helpers\OrganizationUrl::to(['/tests/base/options', 'id'=>$test->id, 'template_id' => $template->id])?>"><?=$template->name."(".$template->infoJson['template'].")"?></a>
                            <?php } ?>
                        <?php } else { ?>
