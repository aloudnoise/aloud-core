<div class="action-content">
    ...
    <div class="row">
        <div class="col">
            ...
            <div class="row">
                <div class="col">
                    <h3><?=Yii::t("main","Назначенные тесты")?></h3>
                </div>
            </div>
            ...
            <div class="mt-3 filter-panel white-block">
                <div class="row">
                    <div class="col-12">
                        <p class="text-muted"><?=Yii::t("main","В этом списке вы можете увидеть полный перечень тестов, которые назначены вам из мероприятий и курсов")?></p>
                    </div>
                </div>
            </div>

            <?php if ($courses OR $tests) { ?>
                <div class="courses-container">
                    <?php foreach ($events as $ev) {
                        ...
                        $event_tests = array_filter($tests, function($t) use ($ev) {
                            return $t->target_id == $ev->id;
                        });

                        if ($event_courses OR $event_tests) { ?>
                            ...
                            <?php if ($event_tests) { ?>
                                <?php foreach ($event_tests as $t) { ?>
                                    ...
                                    <div class="white-block mt-1">
                                        <div class="ml-2">
                                            <?php echo \app\widgets\ETest\ETest::widget([
                                                "model" => $test,
                                                "readonly" => false,
                                                "backbone" => false,
                                                "from" => $process
            ...
            <?php } else { ?>
                <div class="white-block mt-2">
                    <div class="mb-0 alert alert-warning"><h5><?= Yii::t("main", "Тестов не назначено") ?></h5></div>
                </div>
            <?php } ?>
