<?php
$this->setTitle(Yii::t("main","Настройки задания"));
?>
...
<div class="action-content">
    ...
    $f = \app\widgets\EForm\EForm::begin([
        "htmlOptions"=>[
            "action"=>app\helpers\OrganizationUrl::to(array_merge(["/tasks/options"], \Yii::$app->request->get())),
            "method"=>"post",
            "id"=>"taskOptionsForm"
        ],
    ]);
    ...
    <div class="white-block">
        ...
        <div class="form-group" attribute="password">
            <label class="control-label"><?=Yii::t("main","Пароль")?></label>
            <input type="password" name="password" class="form-control" value="<?=$model->password?>" placeholder="<?=Yii::t("main","Пароль")?>" />
        </div>
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <?php if (!empty($criteria_templates)) { ?>
                            <?php foreach ($criteria_templates as $template) { ?>
                                <a target="modal" class="dropdown-item" href="<?=\app\helpers\OrganizationUrl::to(['/tasks/options', 'id'=>$task->id, 'template_id' => $template->id])?>"><?=$template->name."(".$template->infoJson['template'].")"?></a>
                            <?php } ?>
                        <?php } else { ?>
