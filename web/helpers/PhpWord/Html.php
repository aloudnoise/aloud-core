...
class Html
{
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        // Node mapping table
        $nodes = array(                              // $method        $node   $element    $styles     $data   $argument1      $argument2
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(array('app\helpers\PhpWord\Html', $method), $arguments);
    ...
    private static function parseCell($node, $element, &$styles)
    {
        ...
        if ($node->attributes->getNamedItem("rid")->nodeValue)
        {
            ...
            foreach ($rid_a as $rv) {
                $element->addCell(null, array_merge($styles['cell'], [
                    "vMerge" => "continue",
        ...
        return $element->addCell(null, $styles['cell']);
